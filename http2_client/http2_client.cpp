...
Request::Request(uint32_t stream_id, const Variant& uri, zval* data, const Variant& callback, HTTP_METHOD type) {
...
void Http2Client::addRequest(Request* request) {
    this->request_map->insert(map<uint32_t, Request*>::value_type(request->getStreamId(), request));
}
...
static sw_inline void http2_add_header(nghttp2_nv *headers, string key, string value) {
...
void http2_client_send_request(Object& zobject, swClient* cli, Request* request) {
    ...
    int n = http2_client_build_header(zobject, request, buffer + SW_HTTP2_FRAME_HEADER_SIZE,
    ...
    if (post_data == NULL && request->getType() == HTTP_POST) {
        swHttp2_set_frame_header(buffer, SW_HTTP2_TYPE_HEADERS, n, SW_HTTP2_FLAG_END_STREAM | SW_HTTP2_FLAG_END_HEADERS,
                request->getStreamId());
    } else {
    ...
    if (post_data) {
        if (Z_TYPE_P(post_data) == IS_ARRAY) {
            ...
            if (formstr == NULL) {
                swoole_php_error(E_WARNING, "http_build_query post data failed.");
                return;
            }
            memset(buffer, 0, SW_HTTP2_FRAME_HEADER_SIZE);
            swHttp2_set_frame_header(buffer, SW_HTTP2_TYPE_DATA, len, SW_HTTP2_FLAG_END_STREAM, request->getStreamId());
            cli->send(cli, buffer, SW_HTTP2_FRAME_HEADER_SIZE, 0);
            cli->send(cli, formstr, len, 0);
            smart_str_free(&formstr_s);
        } else {
            swHttp2_set_frame_header(buffer, SW_HTTP2_TYPE_DATA, Z_STRLEN_P(post_data), SW_HTTP2_FLAG_END_STREAM,
                    request->getStreamId());
            cli->send(cli, buffer, SW_HTTP2_FRAME_HEADER_SIZE, 0);
...
void http2_client_onFrame(Object& zobject, Object& socket, swClient* cli, char* buf) {
    ...
    if (type == SW_HTTP2_TYPE_SETTINGS) {
        ...
        swWarn("last_stream_id=%d, error_code=%d.", "GOAWAY", last_stream_id, error_code);
        socket.exec("close");
        return;
    }
