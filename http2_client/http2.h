...
#ifndef HTTP2_CLIENT_HTTP2_H_
#define HTTP2_CLIENT_HTTP2_H_
...
enum swHttp2StreamType {
	SW_HTTP2_STREAM_NORMAL = 0, SW_HTTP2_STREAM_PIPELINE = 1,
};
...
static sw_inline uint32_t swHttp2_get_length(char *buf) {
	return (((uint8_t) buf[0]) << 16) + (((uint8_t) buf[1]) << 8)
			+ (uint8_t) buf[2];
}
...
/**
 ...
 +---------------------------------------------------------------+
 */
static void sw_inline swHttp2_set_frame_header(char *buffer, int type,
		int length, int flags, int stream_id) {
	buffer[0] = length >> 16;
	buffer[1] = length >> 8;
	buffer[2] = length;
	buffer[3] = type;
